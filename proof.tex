...
\Fn{highestConfirmedDesc$_v(\bconfirmed, s)$}{
    ...
    \uIf{$\epoch(s) > \epoch(\bcand)$ \textbf{and}
        ...
        $\neg \willNoConflictingChkpBeJustified_v(C(\head, \epoch(s), s))$} {
        \Return{$\bconfirmed$}\;
    }
    \uElse {
        \Return{$\bcand$}\;
...
\Fn{$\varforvalattime[val=v]{\mathit{find\_latest\_confirmed\_descendant}}(b_c)$}
{
        ...
        $\gu(head) \geq \epoch(\varforvalattime[val=v]{t}) - 1$}
    {
        ...
        \uIf{$\epoch(\bcand) = \epoch(\varforvalattime[val=v]{t})$ {\bf or}
...
\begin{definition}[Notation]\leavevmode
    \begin{enumerate}
...
\begin{proof}
Given Lemma's statement 1, we have $\slotstart(\slot(t) - 1) \geq \GST$.
Then Lemma's statement 2 allows us to apply \Cref{lem:vs-at-least-e-2} which implies that
...
By Definition 3 of the paper $\epoch(\gjattime[val=v',time=\slotstartslot{t}])
...
\begin{proof}
Given Lemma's statement 2 and lines xxx of \Cref{alg:conffull} we can conclude that
$\epoch(\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}) \geq \epoch(t)-1$
is the only possible case to satisfy Lemma's statement 3. From this we have
$\epoch(\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bcand}) \geq \epoch(t)-1$.
Then, apply \Cref{lem:gj-at-least-e-2-curr-slot}.
\end{proof}
...
(Sketch). By cases
\begin{description}
    \item[Case 1: {$\epoch(\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}) < \epoch(t)-1$}.]
    Line 15 of \Cref{alg:conffull} must have been executed, otherwise, this case is unreachable.
    From conditions in Line 14 and from Line 17 of \Cref{alg:conffull} we have
    $\varforvalattime[val=v,time=\slotstartslot{t}]{\bcand} \succeq \guattime[val=v,time=\slotstart(\slot(t)-1)]$.
    Also with $\slot(t) = \firstslot(\epoch(t))$ we can conclude that
    $\epoch(\varforvalattime[val=v,time=\slotstartslot{t}]{\bcand}) = \epoch(t)-1$, thus
    $\chkp(\varforvalattime[val=v,time=\slotstartslot{t}]{\bcand}) = \guattime[val=v,time=\slotstart(\slot(t)-1)]$.
    Given $\epoch(\guattime[val=v,time=\slotstart(\slot(t)-1)]) = \epoch(t)-1$ we have
    $\gjattime[val=v,time=\slotstart(\slot(t))] = \guattime[val=v,time=\slotstart(\slot(t)-1)]$.
    Thus Statement 1 of the Lemma is satisfied.
    \item[Case 2: {$\epoch(\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}) \geq \epoch(t)-1$}.]
\end{description}
\end{proof}
