...
\begin{lemma}\label{lem:beta-less-than-quarter-no-reconfirmation-required}
    Let $s' := \firstslot(\epoch(t)+1)$, $\chkp' := \guattime[val=v,time=\slotstart(s'-1)]$, $b := \varforvalattime[val=v,time=\slotstartslot{t})]{\bconfirmed}$.
    If
    \begin{enumerate}
...
\begin{proof}
    Let $s$ be the lowest slot such that $\varforvalattime[val=v,time=\slotstart(s)]{\bconfirmed} = b$. Due to \Cref{lem:bcand-current-epoch-then-no-first-slot} we know that $s > \firstslot(\epoch(t))$, thus $C = \gjattime[val=v,time=\slotstart(s)-1]$ in this case.
    Given $\chkp \preceq \chkp'$ \MKfn{This requires a proof} we conclude the following:
    \begin{itemize}
        ...
        \item $S^{s'-1, v, \slotstart(s'), \chkp'}_{b} = S^{s-1, v, \slotstart(s'), \chkp'}_{b} + S^{s'-1, v, \slotstart(s'), \chkp'}_{s}$ -- because $S^{s'-1, v, \slotstart(s')}_{b} = S^{s-1, v, \slotstart(s')}_{b} \cup S^{s'-1, v, \slotstart(s')}_{s}$.
    \end{itemize}

    Due to \Cref{ln:if-is-one-confirmed-second-loop} we have:
    \begin{align*}
    ...
    \begin{align*}
        ...
        \frac{S^{s'-1, v, \slotstart(s'), \chkp'}_{b}}{W^{s'-1, \chkp'}_{b}} > \frac{1}{2}\left(1 + \frac{W^{\chkp'}_p}{W^{s'-1, \chkp'}_b}\right) + \beta - \frac{D^{s'-1, v, \slotstart(s), \chkp'}_b}{W^{s'-1, \chkp'}_b}\\ \text{-- By definition},\\
        S^{s'-1, v, \slotstart(s'), \chkp'}_{b} > \left(\frac{1}{2} + \beta\right){W^{s'-1, \chkp'}_b} + \frac{1}{2} W^{\chkp'}_p - D^{s'-1, v, \slotstart(s'), \chkp'}_b\\ \text{-- By simplification},\\
        S^{s'-1, v, \slotstart(s'), \chkp'}_{s} + S^{s-1, v, \slotstart(s'), \chkp'}_{b} >\\ \left(\frac{1}{2} + \beta\right){W^{s-1, \chkp'}_b} + \frac{1}{2} W^{\chkp'}_p - D^{s'-1, v, \slotstart(s'), \chkp'}_b + \left(\frac{1}{2} + \beta\right){W^{s'-1, \chkp'}_s}\\ \text{-- By substitution},\\
        S^{s'-1, v, \slotstart(s'), \chkp'}_{s} \geq \left(\frac{1}{2} + \beta\right){W^{s'-1, \chkp'}_s}\\ \text{-- As } S^{s-1, v, \slotstart(s'), \chkp'}_{b} > \left(\frac{1}{2} + \beta\right){W^{s-1, \chkp'}_b} + \frac{1}{2} W^{\chkp'}_p - D^{s'-1, v, \slotstart(s'), \chkp'}_b,\\
...
\begin{proof}
    ...
    Clearly $b = \varforvalattime[val=v,time=\slotstartslot{t'}]{\bconfirmed}$ for some time $t' \leq t$ such that $\epoch(t')=\epoch(t)$ \MKfn{This might need to be proved.}.
...
\begin{proof}
    ...
    By cases.
    \begin{description}
        \item[Case 1: {$\epoch(b') = \epoch(t)-1$}.] Implies  \MKfn{Might need a proof.} that there exists slot $s$ such that $\epoch(s)=\epoch(t)-1$ for which $b' = \varforvalattime[val=v,time=\slotstart(s)]{\bconfirmed}$. Apply \Cref{lem:beta-less-than-quarter-no-reconfirmation-required}.
