...
\Proc{$\varforvalattime[val=v]{\mathit{get\_latest\_confirmed}}(b_c)$}{
    ...
    \uIf{$\epoch(\bcand) < \epoch(\varforvalattime[val=v]{t}) - 1$ {\bf or}
        ...
        {\bf and}
        $\neg (\varforvalattime[val=v]{\mathit{is\_chain\_one\_confirmed}}(b_c, \guattime[val=v,time=\slotstart(\slot(t)-1)]))$ {\bf and} $b_c \succeq \guattime[val=v,time=\slotstart(\slot(t)-1)]$
         {\bf and} 
        $\willNoConflictingChkpBeJustified(\chkp(b_c))$\label{ln:if-bcand-npreceq-head}}{
...
\begin{definition}[\safetyinductionrequirement{s}  for blocks $b$ and $b_\mathsf{s}$, time $t$ and checkpoint $C$ ($\sir(b,t,C,b_\mathsf{s})$)]\label{def:induction-conditions}
    \leavevmode
...
\begin{proof}
    ...
    \begin{description}
        ...
        % Due to \ref{def:induction-conditions:is-lmd-confirmed} and \ref{def:induction-conditions:all-validators} we can apply Lemma~\ref{lem:ffg-condition-on-q-implies-safety} to conclude that $b$ is canonical for any honest validator at time $t'$.
...
\begin{proof}
    ...
    Then, the proof follows from \Cref{lem:gj-prec-prev-confirmed-at-start-of-epoch,lem:conf-prev-epoch-then-vs-two-epochs-ago}.
\end{proof}
...
\begin{proof}(Sketch)
    ...
    \Cref{lem:epoch-gu-less-than-two-epoch-ago-start-epoch} implies $\gjattime[time=t',val=v']  \preceq \varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}$.
    ...
    % This, \Cref{lem:gj-at-least-e-2-curr-slot,lem:gj-does-not-conflict} we can conclude that $\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}$ does not conflict with $\gjattime[time=t',val=v']$.



    % $\epoch(t) > \epoch(\gjattime[time=t',val=v']) \geq \epoch(\guattime[val=v,time={\slotstart(\slot(t)-1)}]) \geq \epoch(t)-2$b


    % Assumption \ref{itm:lem:conf-prev-slot-canonical-at-the-start-of-an-epoch:4} of the Lemma implies that no justified checkpoint for epoch $\epoch(t)-1$ conflicting with $\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}$ can exist.





    % Then, this, \Cref{lem:gu-prec-gj-start-epoch-when-prec-conf-from-at-least-prev-epoch,lem:epoch-gu-less-than-two-epoch-ago-start-epoch}, $\epoch(\gjattime[time=t',val=v']) \geq \epoch(\guattime[val=v,time={\slotstart(\slot(\varforvalattime[val=v]{t})-1)}])$ imply \ref{def:induction-conditions:all-validators:gj-succ} and \ref{def:induction-conditions:all-validators:gj-succ-cs}.

    \ref{def:induction-conditions:is-lmd-confirmed}, \ref{def:induction-conditions:all-validators} and \Cref{lem:ffg-condition-on-q-implies-safety} imply that $\canonical[time=t,blck={\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}},to=\slotstart(\epoch(t)+1)]$. 

    % This then implies that no checkpoint for epoch $\epoch(t)$ conflicting with $\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}$ can ever be justified which concldes the proof for \ref{def:induction-conditions:no-conflicting}.
...
\begin{lemma}\label{lem:output-find-latest-canonical-canonical-at-the-start-of-an-epoch}
    ...
    \begin{enumerate}
        ...
        % \item \ref{def:induction-conditions:ub} holds for a block $b_0$ s.t. $b_0 \preceq b_c$ and $\epoch(b_0) = \epoch(t)-1$ %\varforvalattime[val=v,time=\slotstart(\firstslot(\epoch(t))-1)]{\bconfirmed}
...
\begin{proof}
        We prove first that \ref{def:induction-conditions:is-lmd-confirmed}, \ref{def:induction-conditions:all-validators} for $\sir(\varforvalattime[val=v,time=\slotstart(\slot(t))]{\bconfirmed},t,\varforvalattime[val=v,time=\slotstartslot{t}]{\chkp_{EBA}},b_c)$ hold.\RSfn{This is not really required. It just comes handy rather than writing each single condition explicitly}
        ...
        \ref{def:induction-conditions:is-lmd-confirmed}, \ref{def:induction-conditions:all-validators},   $\canonical[blck={b_c},time=t, to=\slotstart(\epoch(t)+1)]$, and \Cref{lem:ffg-condition-on-q-implies-safety} imply that $\canonical[time=t,blck={\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}},to=\slotstart(\epoch(t)+1)]$. 
        ...
        % \ref{def:induction-conditions:no-conflicting} can be proven as in the proof of \Cref{lem:conf-prev-slot-canonical-at-the-start-of-an-epoch}: from the proof of the previous conditions, we can conclude that $\canonical[time=t,blck={b_o},to=\slotstart(\epoch(t)+1)]$ from which we can prove that  no checkpoint for epoch $\epoch(t) = \epoch(b_o)+1$ conflicting with ${b_o}$ can ever be justified.
        % \ref{def:induction-conditions:ub} holding for ${b_o}$ implies \ref{def:induction-conditions:no-conflicting}  for epoch $\epoch(b) = \epoch(t)-1$, 
\end{proof}
...
\begin{proof}
    ...
    \end{enumerate}
    \begin{description}
        \item[Base Case: $\slot(\tind) = \firstslot(\epoch(t))$.]  \Cref{lem:confirmed-end-first-slot-canonical} proves that  $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t'))]{\bconfirmed}},time=t,to=\slotstart(\epoch(t)+1)]$.
        ...
        Otherwise, from \Cref{lem:if-new-bc-then-output-find-latest} we know that\\ $\varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}=\mathit{find\_latest\_confirmed\_descendant}(\varforvalattime[val=v,time=\slotstart(\slot(\tind)-1)]{\bconfirmed})$.
        ...
        This, together with the addditional inducive hypothesis allows us to apply \Cref{lem:output-find-latest-canonical-canonical-at-the-start-of-an-epoch} and conclude the proof.
        ...
        % \ref{def:induction-conditions:ub} holding for ${\varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}}$ implies \ref{def:induction-conditions:no-conflicting}  for epoch $\epoch(b) = \epoch(t)-1$, 
    \end{description}
\end{proof}
...
\begin{lemma}\label{lem:no-conflicting-to-c-b-at-the-start-of-an-epoch}
    ...
    \begin{enumerate}
        ...
        \item  $t = \slotstart(\epoch(t))$
        \item the \KwSty{if} condition at line \Cref{ln:if-bcand-npreceq-head} is false
        % \item\label{itm:lem:conf-prev-slot-canonical-at-the-start-of-an-epoch:4} \ref{def:induction-conditions:ub} holds for $\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}$
        % \item $\slotstart(\lastslot(\epoch(t)-1)) \geq \GST$
        % \item $\epoch(b_c) \geq \epoch(t)-1$
        % \item $b_c \in \filtered[val=v',time=t']$
    \end{enumerate},
...
\begin{proof}
    ...
    From \Cref{lem:epoch-gu-less-than-two-epoch-ago-start-epoch} we know that $\epoch(\guattime[time=\slotstart(\slot(t)-1),val=v]) \in \{\epoch(t-2),\epoch(t)-1\}$.
    ...
    From \Cref{lem:no-curr-epochconflict-chkp-is-justified}, we know that there exists a time $t' \in \{\slotstart(s): s \in [\firstslot(\epoch(t)-1),\lastslot(\epoch(t)-1)]\}$ such that $\varforvalattime[val=v,time=t']{\willChkpBeJustified}(\varforvalattime[val=v,time=t']{\bconfirmed})$.

    \Cref{lem:conf-beginning-prec-bcand} then implies that $\varforvalattime[val=v,time=t']{\bconfirmed} \preceq \varforvalattime[val=v,time=\slotstart(\firstslot(\epoch(t))-1)]{\bconfirmed}$.
    ...
    We know that this implies\RSfn{TBD} that $\epoch(\varforvalattime[val=v,time=\slotstart(\firstslot(\epoch(t))-2)]{\bconfirmed}) = \epoch(t)-2$.
    ...
    Hence, we can apply \Cref{lem:canonical-for-current-epoch-with-extra-assum} to conclude that $\canonical[blck={\varforvalattime[val=v,time=t']{\bconfirmed}},time=t',to=\slotstart(\epoch(t))]$.
...
\begin{proof}(Sketch)
    \Cref{lem:conf-prev-slot-canonical-at-the-start-of-an-epoch,lem:no-conflicting-to-c-b-at-the-start-of-an-epoch} imply that \ref{def:induction-conditions:is-lmd-confirmed}, \ref{def:induction-conditions:all-validators} for $\sir(\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed},t,\guattime[val=v,time={\slotstart(\slot({t})-1)}],\block(\guattime[val=v,time={\slotstart(\slot(\varforvalattime[val=v]{t})-1)}]))$ hold, $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}},time=t,to=\slotstart(\epoch(t)+1)]$ and that for any honest validators $v'$ and time $t'$ s.t. $\epoch(t') = \epoch(t)$, $\gjattime[time=t',val=v']  \preceq \varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}$ and $\gjattime[time=t',val=v']  \succeq \guattime[time=\slotstart(\firstslot(\epoch(t))-1),val=v]$.
    ...
    All of this allows us to apply \Cref{assm:bcand-chkp-in-allu} to conclude \ref{def:induction-conditions:ub}.

     \Cref{lem:no-conflicting-to-c-b-at-the-start-of-an-epoch} and $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}},time=t,to=\slotstart(\epoch(t)+1)]$ also imply \ref{def:induction-conditions:no-conflicting}.

    Now, \Cref{lem:ffg-safety-from-sir} imply  $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}},time=t]$
    % Then, given $\varforvalattime[val=v,time=t']{\bconfirmed} \preceq \varforvalattime[val=v,time=\slotstart(\firstslot(\epoch(t))-1)]{\bconfirmed}$ and  $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}},time=t,to=\slotstart(\epoch(t)+1)]$, $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed}},time=t,to=\slotstart(\epoch(t)+1)]$.
\end{proof}
...
\begin{lemma}\label{lem:output-find-latest-canonical-canonical-at-the-start-of-an-epoch-for-ever}
    ...
    \begin{enumerate}
        ...
        % \item for any honest validators $v'$ and time $t'$ s.t. $\epoch(t') = \epoch(t)$, $\gjattime[time=t',val=v']  \preceq b_c$
        % \item  for any honest validators $v'$ and time $t'$ s.t. $\epoch(t') = \epoch(t)$, $\gjattime[time=t',val=v']  \succeq \guattime[time=\slotstart(\firstslot(\epoch(t))-1),val=v]$
        % \item \ref{def:induction-conditions:ub} holds for a block $b_0$ s.t. $b_0 \preceq b_c$ and $\epoch(b_0) = \epoch(t)-1$ %\varforvalattime[val=v,time=\slotstart(\firstslot(\epoch(t))-1)]{\bconfirmed}
        % \item $\epoch(b_c) \geq \epoch(\varforvalattime[val=v]{t})-1$
        \item $t = \slotstart(\slot(t))$
...
\begin{proof}
    \Cref{lem:conf-prev-slot-canonical-at-the-start-of-an-epoch-no-extra-assum,lem:output-find-latest-canonical-canonical-at-the-start-of-an-epoch} imply \ref{def:induction-conditions:is-lmd-confirmed}, \ref{def:induction-conditions:all-validators} for $\sir(\varforvalattime[val=v,time=\slotstart(\slot(t))]{\bconfirmed},t,\varforvalattime[val=v,time=\slotstartslot{t}]{\chkp_{EBA}},\varforvalattime[val=v,time=\slotstart(\slot(t)-1)]{\bconfirmed})$ hold and  $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t))]{\bconfirmed}},time={\varforvalattime[val=v]{t}},to=\slotstart(\epoch(t)+1)]$
    ...
    % Then, 
     ...
     \Cref{lem:no-conflicting-to-c-b-at-the-start-of-an-epoch} and $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t))]{\bconfirmed}},time=t,to=\slotstart(\epoch(t)+1)]$ also imply \ref{def:induction-conditions:no-conflicting}.

    Now, \Cref{lem:ffg-safety-from-sir} imply  $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t))]{\bconfirmed}},time=t]$    
\end{proof}
...
\begin{proof}
    It should be easy to conclude from \Cref{lem:conf-prev-slot-canonical-at-the-start-of-an-epoch-no-extra-assum,lem:gu-e-1-canonical}.
\end{proof}
...
\begin{proof}
    ...
    \end{enumerate}
    \begin{description}
        \item[Base Case: $\slot(\tind) = \firstslot(\epoch(t))$.]  \Cref{lem:confirmed-end-first-slot-canonical-no-extra-assum} proves that  $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(t'))]{\bconfirmed}},time=t]$.
        \item[Inductive Step: $\slot(\tind) > \firstslot(\epoch(t))$.] 
        Let $v'$ be any honest validator and $t' \in [t, \slotstart(\epoch(t)+1)]$.
        ...
        \begin{description}
            ...
            The above, together with the additional inductive hypothesis allows us to apply \Cref{lem:output-find-latest-canonical-canonical-at-the-start-of-an-epoch} which implies that $\canonical[blck={ \varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}},time=\tind,to=\slotstart(\epoch(t)+1)]$ and 
            ...
            Now, \Cref{lem:ffg-safety-from-sir} imply  $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}},time=t]$    
            \item [Case 2: ${\epoch(\varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed})=\epoch(t)}$.] 
            We can use \Cref{lem:output-find-latest-canonical-canonical-at-the-start-of-an-epoch} to conclude \ref{def:induction-conditions:is-lmd-confirmed}.
            \begin{description}
                \item[$\epoch(t')=\epoch(t)$.]
                We can use \Cref{lem:output-find-latest-canonical-canonical-at-the-start-of-an-epoch} to conclude \ref{def:induction-conditions:all-validators} and $\canonical[blck={ \varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}},time=\tind,to=\slotstart(\epoch(t)+1)]$.
                % From the inductive hypothesis \ref{itm:lem:canonical-no-extra-assum:ind-1} and \Cref{lem:conf-beginning-prec-bcand}, we can conclude that $\gjattime[time=t',val=v']  \preceq \varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}$.
                % \Cref{lem:conf-prev-epoch-then-vs-two-epochs-ago} then implies \ref{def:induction-conditions:all-validators:not-filtered-out}.
                \item[$\epoch(t')=\epoch(t)+1$.]  
                \Cref{lem:no-curr-epochconflict-chkp-is-justified} and the inductive hypothesis, and $\canonical[blck={ \varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}},time=\tind,to=\slotstart(\epoch(t)+1)]$ ensure that any checkpoint $C$ s.t. is justified and $\epoch(C)=\epoch(t)$, then $C = \chkp( \varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed})$.
                ...
                \Cref{lem:conf-prev-epoch-then-vs-two-epochs-ago} then implies that \ref{def:induction-conditions:all-validators} holds.
                Then, \Cref{lem:ffg-condition-on-q-implies-safety} implies that $\canonical[blck={ \varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}},time=\tind,to=\slotstart(\epoch(t)+2)]$.
                All the above then implies that \ref{def:induction-conditions:no-conflicting} holds.
                Finally, \Cref{lem:no-curr-epochconflict-chkp-is-justified}, $\canonical[blck={ \varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}},time=\tind,to=\slotstart(\epoch(t)+2)]$, \Cref{lem:if-new-bc-then-output-find-latest} imply \ref{def:induction-conditions:ub}.
            \end{description}
            Now, \Cref{lem:ffg-safety-from-sir} imply  $\canonical[blck={\varforvalattime[val=v,time=\slotstart(\slot(\tind))]{\bconfirmed}},time=\tind]$.
        \end{description}
