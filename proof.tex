...
\begin{proof}
    ...
    \begin{description}
        ...
        If $\bcand$ is set to $b_o$ here, then it must be that $b_o \preceq phead$ which also implies $b_o \in \viewattime[time=\slotstart({\varforvalattime[val=v]{t}}-1),val=v] \subseteq \viewattime[time=\slotstart({\varforvalattime[val=v]{t}}),val=v]$.
...
\begin{proof}
    ...
    \begin{description}
        ...
        %     \item[Case 1.2: $\epoch(b_c) = \epoch(s)-1$.] 
            We can apply \Cref{lem:no-prev-epochconflict-chkp-is-justified} to conclude that $\gjattime[time=\slotstartslot{t},val=v']$ does not conflict with $b_c$.
        % \end{description}
        ...
        Given that $b' \succeq b_c$, $\gjattime[time=\slotstartslot{t},val=v']$ does not conflict with $b_c$.
    \end{description}
\end{proof}
