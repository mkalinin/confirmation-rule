...
\Proc{$\varforvalattime[val=v]{\mathit{get\_latest\_confirmed}}(b_c)$}{
    ...
    \uIf{$\slot(\varforvalattime[val=v]{t}) = \firstslot(\epoch(\varforvalattime[val=v]{t}))$ {\bf and}
        $\epoch(\guattime[val=v,time={\slotstart(\slot(\varforvalattime[val=v]{t})-1)}]) = \epoch(\varforvalattime[val=v]{t})$ {\bf and}
        $\slot(\bcand) < \slot(\guattime[val=v,time={\slotstart(\slot(\varforvalattime[val=v]{t})-1)}])$}
    {
...
Given all this we have $b' \in \viewattime[time=\slotstart(\slot(t)),val=v']$. By cases
\begin{description}
    \item[Case 1: {$\epoch(b') < \epoch(t)$}.]
    Thus $\votsource[blck=b',time=\slotstartslot{t}] = \gu(b') \geq \epoch(t) - 2$.
    Clearly $\epoch(\gjattime[val=v',time=\slotstartslot{t}]) \geq \epoch(\gu(b')) \geq \epoch(t) - 2$ in this case.
    \item[Case 2: {$\epoch(b') = \epoch(t)$}.]
    Thus $\votsource[blck=b',time=\slotstartslot{t}] = \gjblock(b') \geq \epoch(t) - 2$.
    Clearly $\epoch(\gjattime[val=v',time=\slotstartslot{t}]) \geq \epoch(\gjblock(b')) \geq \epoch(t) - 2$ in this case.
\end{description}
\end{proof}
...
\begin{proof}
Given Lemma's statements 2 and 3, and lines xxx of \Cref{alg:conffull} we can conclude that
...
(Sketch). By cases
\begin{description}
    \item[Case 1: {$\epoch(\varforvalattime[val=v,time=\slotstartslot{t}]{\bcand}) = \epoch(t)$}.]
    From Line 23 of \Cref{alg:findlatestconf} follows that Statement 2 holds.
    \item[Case 2: {$\epoch(\varforvalattime[val=v,time=\slotstartslot{t}]{\bcand}) = \epoch(t) - 1$}.]
\end{description}
\end{proof}
