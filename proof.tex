...
\begin{lemma}\label{lem:output-find-latest-different-to-input}
    ...
    \begin{enumerate}
        \item $b_o \neq b_c$
        \item $\epoch(b_o) \leq \epoch(t^v) - 1$
        \item $\epoch(b_c) \geq \epoch(t^v) - 2$
    \end{enumerate},
    then, there exists an honest validator $v'$ such that 
    \begin{enumerate}
        \item $\exists b' \in \left(\viewattime[time=\slotstart(\slot({\varforvalattime[val=v]{t}})-1),val=v'] \cap \viewattime[time=\slotstart(\slot({\varforvalattime[val=v]{t}})),val=v]\right), b_o \succ \votsource[blck=b',time=\slotstartslot{t^v}] \land \epoch(\votsource[blck=b',time=\slotstartslot{{\varforvalattime[val=v]{t}}}])\geq \epoch({\varforvalattime[val=v]{t}})-2$
