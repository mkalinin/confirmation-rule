...
\begin{lemma}\label{lem:gu-prev-slot-descendant-of-gj-prev-slot}
    Pick any $b_c \in \varforvalattime[val=v,time=\slotstart(\slot(s))]{\bcands}$ for any slot $s$ such that $\epoch(s) = \epoch(t)$.
    If
    \begin{enumerate}
        \item $\firstslot(\epoch(t)-1) \geq \GST$
        \item $\epoch(b_c) = \epoch(t)-1$
    ...
    then,
    \begin{enumerate}
        \item $\gjattime[val=v,time=\slotstart(s-1)] \preceq \guattime[val=v,time=\slotstart(\slot(t)-1)]$
    \end{enumerate}
\end{lemma}
...
\begin{proof}
    Let $C := C_{EBA}$.
    Clearly $b \in \varforvalattime[val=v,time=\slotstart(s)]{\bcands}$ for some slot $s \leq \slot(t)$ such that $\epoch(s)=\epoch(t)$ \MKfn{This might need to be proved.}.
    Due to \Cref{lem:bcand-current-epoch-then-no-first-slot} we know that $s > \firstslot(\epoch(t))$, thus $C = \gjattime[val=v,time=\slotstart(s)-1]$ in this case.
    Then because of \Cref{lem:gu-prev-slot-descendant-of-gj-prev-slot} we have $\chkp = \gjattime[val=v,time=\slotstart(s-1)] \preceq \guattime[val=v,time=\slotstart(\slot(t)-1)] = \chkp'$. Given that we can conclude the following:
    \begin{itemize}
