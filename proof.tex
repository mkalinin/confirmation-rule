...
\begin{lemma}\label{lem:output-find-latest-different-to-input}
    ...
    \begin{enumerate}
        ...
        \RSfn{Must be at the voting time of $\slot(\varforvalattime[val=v]{t})-1$ not at the beginning.}
    \end{enumerate}    
\end{lemma}
...
\begin{proof}
    ...
    \begin{description}
        ...
        Then, given \Cref{ln:if-prev-epoch} and the Lemma statement, $b_o \succ \votsource[blck=phead,time={\slotstartslot{\varforvalattime[val=v]{t}}}] \land \votsource[blck=phead, time={\slotstartslot{\varforvalattime[val=v]{t}}}] \geq \epoch({\varforvalattime[val=v]{t}}) - 2$.
        \item[\Cref{ln:set-bcand-to-btcand}] If $\bcand$ is set to $b_o$ here, then, due to \Cref{ln:if-to-set-bcand-to-btcand}, $\votsource[blck=b_o, time={\slotstartslot{\varforvalattime[val=v]{t}}}] \geq \epoch(\varforvalattime[val=v]{t}) - 2$.
            ...
            \begin{description}
                ...
                \begin{description}
                    ...
                    \begin{description}
                        ...
                        Obvious from \Cref{lem:output-find-latest-different-to-input}.
                        \item[Case 2.2.2: $\epoch(b_c) \neq \epoch(s+1) - 1$.]
                        By the assumption in the Lemma statement, it must be that $\epoch(b_c) = \epoch(s+1)$.
                        From \Cref{lem:bcand-current-epoch-then-no-first-slot,lem:prev-conf-at-least-e-1}, we can conclude that $b_c \succeq \varforvalattime[val=v,time=\slotstart(s)]{\bconfirmed}$.
...
\begin{proof}
    Let $s := \slot(t)$.
    Due to \Cref{lem:vs-at-least-e-2}, we know that there exists an honest validator $v_h$ such that $\exists b' \in \viewattime[time=\slotstart(s-1),val=v_h], b_c \succ \votsource[blck=b',time=\slotstart(s)] \land \epoch(\votsource[blck=b',time=\slotstart(s)])\geq \epoch(s)-2$.
...
\begin{proof}
    By cases.
    From \Cref{lem:conf-prev-epoch-then-vs-two-epochs-ago}, we know that $\exists b' \in \viewattime[time=\slotstart(s-1),val=v], \epoch(\votsource[blck=b',time=\slotstart(s-1)])\geq \epoch(s-1)-2 \land \varforvalattime[val=v,time=\slotstart(s-1)]{\bconfirmed} \succ \votsource[blck=b',time=\slotstart(s-1)]$.
    ...
    \begin{description}
        ...
        \begin{description}
            ...
            Because $\epoch(\votsource[blck=b',time=\slotstart(s-1)]) = \epoch(s-1) -2 < \epoch(s-1) -1 \leq \epoch(s)-1 \leq \epoch(\varforvalattime[val=v,time=\slotstart(s-1)]{\bconfirmed})$,  $\votsource[blck=b',time=\slotstart(s-1)] \prec \varforvalattime[val=v,time=\slotstart(s-1)]{\bconfirmed}$, we can apply \Cref{prop:on-chkp-prec} to conclude that $\votsource[blck=b',time=\slotstart(s-1)] \preceq \chkp(\varforvalattime[val=v,time=\slotstart(s-1)]{\bconfirmed},\epoch(s-1)-1)$.
        ...
        \begin{description}
            ...
            Because $\epoch(\votsource[blck=b',time=\slotstart(s-1)]) = \epoch(s-1) -2 < \epoch(s-1) = \epoch(\varforvalattime[val=v,time=\slotstart(s-1)]{\bconfirmed})$,  $\votsource[blck=b',time=\slotstart(s-1)] \prec \varforvalattime[val=v,time=\slotstart(s-1)]{\bconfirmed}$, we can appy \Cref{prop:on-chkp-prec} which, together with $\epoch(\gjattime[time=\slotstart(s),val=v']) = \epoch(s-1)$, allows us to conclude that $\gjattime[time=\slotstart(s-1),val=v] = \votsource[blck=b',time=\slotstart(s-1)] \preceq \chkp(\varforvalattime[val=v,time=\slotstart(s-1)]{\bconfirmed},\epoch(s-1))= \chkp(\varforvalattime[val=v,time=\slotstart(s-1)]{\bconfirmed})= \gjattime[time=\slotstart(s),val=v']$.
...
\begin{proof}
From \Cref{lem:conf-prev-epoch-then-vs-two-epochs-ago} we know that there exists an honest validator $v_h$ such that $\exists b' \in \viewattime[time=\slotstart(\slot(t)-1),val=v_h], \epoch(\votsource[blck=b',time=\slotstartslot{t}])\geq \epoch(t)-2$.
...
\begin{lemma}\label{lem:gu-e-1-canonical}
    If
    \begin{enumerate}
        \item \RS{$t = \firstslot(\epoch(t))$}
        \item $\slotstart(\slot(t)-1) \geq \GST$
        \item $\epoch(\guattime[val=v,time=\slotstart(\slot(t)-1)]) = \epoch(t)-1$
    \end{enumerate},
    then,
    \begin{enumerate}
        \item $\canonical[blck={\block(\guattime[val=v,time=\slotstart(\slot(t)-1)])},time=t]$.
    \end{enumerate}
\end{lemma}

\begin{proof}
    (Sketch)
    Given the synchrony assumption $\gjattime[val=v',time=t'] = \guattime[val=v,time=\slotstart(\slot(t)-1)]$ for any honest validator $v'$ and time $t'$ such that $\epoch(t') = \epoch(t)$.
    Then all honest validators will cast their FFG votes with $\guattime[val=v,time=\slotstart(\slot(t)-1)]$ as the source throughout $epoch(t)$.
    Therefore, $\gfattime[val=v',time=\slotstart(\epoch(t)+1)] = \guattime[val=v,time=\slotstart(\slot(t)-1)]$.
\end{proof}

\begin{lemma}\label{lem:canonical-at-st-t}
    Pick any $b_c \in  \varforvalattime[val=v,time=\slotstart(\slot(t))]{\bcands}$.
    If
    \begin{enumerate}
        \item $\slotstart(\lastslot(\epoch(t)-2)) \geq \GST$
        \item $\epoch(b_c) \geq \epoch(t)-1$
    \end{enumerate},
    then,
    \begin{enumerate}
        \item $b_c \preceq \LMDGHOSTHFC(\viewattime[val=v',time=\slotstartslot{t}])$ for any honest validator $v'$.
    \end{enumerate}
\end{lemma}
...
\begin{proof}
    ...
    \begin{description}
        \item[Base case: {$s = \slot(\varforvalattime[val=v]{\tinit})$}.] The Lemma is vacuously true.
        \item[Inductive case: {$s \geq \slot(\varforvalattime[val=v]{\tinit})$}.] We assume that the Lemma holds for slot $s$ and prove that it also holds for $s+1$.
        Now by cases on the first line, in order of execution of the Fast Confirmation Rule algorithm at time $\slotstart(s+1)$, where $\bcand = b_c$. 
        \begin{description}
            \item[\Cref{ln:set-bcand-beginning-of-get-latest-confirmed}.] By the inductive hypothesis we know that $b_c \preceq \LMDGHOSTHFC(\viewattime[val=v',time=\slotstart(s)])$.
            Thus all honest validators voted in the support of $b_c$ in slot $s$. This together with the synchrony assumption implies $b_c \preceq \LMDGHOSTHFC(\viewattime[val=v',time=\slotstart(s+1)])$.
            \item[\Cref{ln:set-bcand-to-fin}.] Impossible, as at this line $\epoch(\bcand) < \epoch(s+1)-1$, but we assume $\epoch(b_c) \geq \epoch(s+1)-1$.
            \item[\Cref{ln:set-bcan-on-start-conf-chain}.] Apply \Cref{lem:gu-e-1-canonical}.
            \item[\Cref{ln:set-bcand-to-output-find-latest}.] TBD.
        \end{description}
    \end{description}
\end{proof}
...
\begin{proof}
    ...
    \begin{description}
        ...
        \begin{description}
            ...
            Then \Cref{lem:gj-at-least-e-2-curr-slot} implies $\epoch(t)-2 = \epoch(\gjattime[val=v',time=t'']) < \epoch(\gjattime[val=v',time=t']) = \epoch(t)-1$.
            Due to \Cref{lem:no-prev-epochconflict-chkp-is-justified} and the synchrony assumption we know that no checkpoint conflicting with $\chkp(b_c, \epoch(t)-1)$ can be justified for $\epoch(t)-1$.
            Thus, $\gjattime[val=v',time=t'] = \chkp(b_c, \epoch(t)-1) \preceq b_c$.
        \end{description}
    \end{description}
\end{proof}

\begin{lemma}\label{lem:not-filtered-during-e}
    Pick any $b_c \in  \varforvalattime[val=v,time=\slotstart(\slot(t))]{\bcands}$, any honest validator $v'$ and time $t' \geq \slotstartslot{t}$ such that $\epoch(t') = \epoch(t)$.
    If
    \begin{enumerate}
        \item $\slotstart(\epoch(t)-1) \geq \GST$
...
\begin{proof}
    From \Cref{lem:gj-does-not-conflict-during-e} we know that $b_c$ does not conflict with $\gjattime[val=v',time=t']$.
    Then \Cref{lem:conf-prev-epoch-then-vs-two-epochs-ago} together with the synchrony assumption implies $\exists b' \in \viewattime[time=t',val=v'], b_c \succ \votsource[blck=b',time=t'] \land \epoch(\votsource[blck=b',time=t'])\geq \epoch(t')-2$ which concludes the proof.
\end{proof}

\begin{lemma}\label{lem:canonical-during-e}
    Pick any $b_c \in  \varforvalattime[val=v,time=\slotstart(\slot(t))]{\bcands}$, any honest validator $v'$ and time $t' \geq \slotstartslot{t}$ such that $\epoch(t') = \epoch(t)$.
    If
    \begin{enumerate}
        \item $\slotstart(\lastslot(\epoch(t)-2)) \geq \GST$
...
\begin{proof}
    ...
    By induction on $t'$.
    \begin{description}
        \item[Base case: {$t' = \slotstartslot{t}$}.] Apply \Cref{lem:canonical-at-st-t}. 
        \item[Inductive case: {$t' \geq \slotstartslot{t}$}.] We assume that the Lemma holds for any $t''$ such that $\slotstartslot{t} \leq t'' < t'$ and we prove that it holds at time $t'$ as well.
        Consider $\varforvalattime[val=v,time=t]{\isOneConfirmed}(b_c, \chkp)$ such that $\chkp \in \left\{\guattime[val=v,time=\slotstart(\slot(t)-1)],\gjattime[val=v,time=\slotstart(\slot(t)-1)]\right\}$, show that $\chkp \preceq \gjattime[val=v',time=t']$.
        Then use Lemmas from the Paper to show that $b_c$ outweighs all other blocks at any slot of $\epoch(t')$ and finally apply \Cref{lem:not-filtered-during-e} to conclude the proof.
    \end{description}
\end{proof}
...
\begin{proof}
    ...
    By induction on $t'$.
    \begin{description}
        \item[Base case: {$\epoch(t') = \epoch(t)$}.] Apply \Cref{lem:gj-does-not-conflict-during-e}.
        \item[Inductive case: {$\epoch(t') = \epoch(t)+1$}.] We assume that the Lemma holds for any $t'' < t'$ and we prove that it holds at time $t'$ as well.
        By cases.
        \begin{description}
            \item[Case 1: {$\epoch(\gjattime[val=v',time=t']) = \epoch(\gjattime[val=v',time=t''])$}.] Apply the inductive hypothesis.
            \item[Case 2: {$\epoch(\gjattime[val=v',time=t']) \neq \epoch(\gjattime[val=v',time=t''])$}.] This \RSfn{and the synchrony assumption} and \Cref{lem:gj-at-least-e-2-curr-slot} implies $\epoch(t)-2 \leq \epoch(\gjattime[val=v',time=t'']) < \epoch(\gjattime[val=v',time=t']) \leq \epoch(t)$.
            By sub cases.
            \begin{description}
                \item[Case 1.1: {$\epoch(\gjattime[val=v',time=t']) = \epoch(t)-1$}.]
                Due to \Cref{lem:no-prev-epochconflict-chkp-is-justified} and the synchrony assumption we know that no checkpoint conflicting with $\chkp(b_c, \epoch(t)-1)$ can be justified for $\epoch(t)-1$.
                Thus $\gjattime[val=v',time=t'] = \chkp(b_c, \epoch(t)-1) \preceq b_c$.
                \item[Case 1.2: {$\epoch(\gjattime[val=v',time=t']) \neq \epoch(t)-1$}.] Implies $\epoch(\gjattime[val=v',time=t']) = \epoch(t)$.
                By sub cases again.
                \begin{description}
                    \item[Case 1.2.1: {$\epoch(b_c) = \epoch(t)-1$}.] By sub cases again.
                    \begin{description}
                        \item[Case 1.2.1.1: {$\slot(t) = \firstslot(\epoch(t))$}.]
                        From \Cref{lem:canonical-during-e} we know that $b_c \preceq \LMDGHOSTHFC(\viewattime[val=v',time=t'''])$, $\epoch(t''') = \epoch(t)$,
                        thus all honest validators in the committees of $\epoch(t)$ will send their FFG votes targeting a checkpoint that is a descendant \RSfn{ancestor} of $b_c$.
                        Then Property 1.6 of the Paper together with the synchrony assumption implies that no checkpoint $\chkp'$
                        such that $\epoch(\chkp') = \epoch(t)$ conflicting with $b_c$ can ever be justified.
                        \item[Case 1.2.1.2: {$\slot(t) \neq \firstslot(\epoch(t))$}.] By contradiction.
                        Suppose $\exists b', \chkp(b',\epoch(t)) \in \allU(\viewattime[val=v',time=t'])$ and $b_c \npreceq \chkp(b',\epoch(t))$.
                        Then, given the synchrony assumption and $\epoch(b_c)=\epoch(t)-1$, $\willNoConflictingChkpBeJustified_v(\chkp(b', \epoch(t)))$ check in \Cref{ln:if-prev-epoch} and \Cref{ln:if-to-set-bcand-to-btcand} must have never be true,
                        thus either $\slot(t) = \firstslot(\epoch(t))$ or $\epoch(b_c) \neq \epoch(t)-1$.
                    \end{description}
                    \item[Case 1.2.2: {$\epoch(b_c) \neq \epoch(t)-1$}.] Implies $\epoch(b_c) = \epoch(t)$.
                    Due to \Cref{lem:no-curr-epochconflict-chkp-is-justified} and the synchrony assumption, no checkpoint $\chkp'$ such that $\epoch(\chkp') = \epoch(t)$ conflicting with $b_c$ can ever be justified.
                    Thus $\gjattime[val=v',time=t'] = \chkp(b_c, \epoch(t)) \preceq b_c$.
                \end{description}
            \end{description}
        \end{description}
    \end{description}
...
\begin{lemma}\label{lem:not-filtered-during-e+1}
    Pick any $b_c \in  \varforvalattime[val=v,time=\slotstart(\slot(t))]{\bcands}$, any honest validator $v'$ and time $t' \geq \slotstartslot{t}$ such that $\epoch(t') \in \left\{\epoch(t), \epoch(t)+1\right\}$.
    If
    \begin{enumerate}
        \item $\slotstart(\lastslot(\epoch(t)-2)) \geq \GST$
...
\begin{proof}
    (Sketch)
    From \Cref{lem:gj-does-not-conflict-during-e+1} we know that $b_c$ does not conflict with $\gjattime[val=v',time=t']$.
    By cases.
    \begin{description}
        \item[Case 1: {$\epoch(t') = \epoch(t)$}.] Apply \Cref{lem:not-filtered-during-e}.
        \item[Case 2: {$\epoch(t') = \epoch(t)+1$}.] 
        By sub cases.
        \begin{description}
            \item[Case 2.1: {$\slot(t) = \firstslot(\epoch(t))$}.]
            From \Cref{lem:canonical-during-e} we know that $b_c \preceq \LMDGHOSTHFC(\viewattime[val=v',time=t''])$, $\epoch(t'') = \epoch(t)$,
            thus all honest validators in the committees of $\epoch(t)$ will send their FFG votes targeting a checkpoint that is a descendant of $b_c$.
            Then Property 1.6 of the Paper together with the synchrony assumption implies that $\gjattime[val=v',time=\slotstart(\epoch(t'))] = \chkp(b',\epoch(t')-1)$ such that $b' \succeq b_c$. Then obviously $b_c \in \filtered[val=v',time=t']$.
            \item[Case 2.2: {$\slot(t) \neq \firstslot(\epoch(t))$}.]
            If statements at \Cref{ln:if-prev-epoch} and \Cref{ln:second-if} ensure that $\exists b' \in \viewatstslottime[val=v,time=t], b' \succeq b_c$ such that $\gu(b') \geq \epoch(t)-1 = \epoch(t')-2$, thus $\epoch(\votsource[blck=b',time=\epoch(t')]) = \epoch(\gu(b')) \geq \epoch(t')-2$.
            This together with the synchrony assumption implies $\exists b' \in \viewattime[time=t',val=v'], b' \succeq b_c$ such that $\epoch(\votsource[blck=b',time=\epoch(t')]) \geq \epoch(t')-2$ concluding the proof.
        \end{description}
    \end{description}
\end{proof}

\begin{lemma}\label{lem:canonical-during-e+1}
    Pick any $b_c \in  \varforvalattime[val=v,time=\slotstart(\slot(t))]{\bcands}$, any honest validator $v'$ and time $t' \geq \slotstartslot{t}$ such that $\epoch(t') \in \left\{\epoch(t),\epoch(t)+1\right\}$.
    If
    \begin{enumerate}
        \item $\slotstart(\lastslot(\epoch(t)-2)) \geq \GST$
...
\begin{proof}
    ...
    By induction on $t'$.
    \begin{description}
        \item[Base case: {$\epoch(t') = \epoch(t)$}.] Apply \Cref{lem:canonical-during-e}.
        \item[Inductive case: {$\epoch(t') = \epoch(t)+1$}.] We assume that the Lemma holds for any $t'' < t'$ and we prove that it holds at time $t'$ as well.
        Consider $\varforvalattime[val=v,time=t]{\isOneConfirmed}(b_c, \chkp)$ such that $\chkp \in \left\{\guattime[val=v,time=\slotstart(\slot(t)-1)],\gjattime[val=v,time=\slotstart(\slot(t)-1)]\right\}$, show that $\chkp \preceq \gjattime[val=v',time=t']$.
        Then use Lemmas from the Paper to show that $b_c$ outweighs all other blocks at any slot of $\epoch(t)+1$.
        Finally, \Cref{lem:not-filtered-during-e+1} concludes the proof.
    \end{description}
\end{proof}
...
\begin{proof}
    (Sketch)
    From \Cref{lem:canonical-during-e+1} we know that $b_c \preceq \LMDGHOSTHFC(\viewattime[val=v',time=t'])$
    for any honest validator $v'$ and time $t' \geq \slotstartslot{t}$ such that $\epoch(t') \in \left\{\epoch(t), \epoch(t)+1\right\}$.
    Thus all honest validators in the committees of $\epoch(t)+1$ will send their FFG votes targeting checkpoint $\chkp(b', \epoch(t)+1)$ such that $b' \succeq b_c$.
    Then Property 1.6 of the Paper together with the synchrony assumption implies $\gjattime[val=v',time=\slotstart(\epoch(t)+2)] = \chkp(b', \epoch(t)+1)$.
    This implies $b'$ remaining canonical throughout $epoch(t)+2$. Finally, by following the same reasoning show that $\gfattime[val=v',time=\slotstart(\epoch(t)+3)] = \chkp(b', \epoch(t)+1)$.
\end{proof}
...
\begin{proof}
    ...
    \begin{description}
        ...
        \begin{description}
            ...
            \item[Case 2: {$\epoch(\gjattime[val=v',time=t']) \neq \epoch(\gjattime[val=v',time=t''])$}.]
            \Cref{lem:gj-at-least-e-2-curr-slot} implies $\epoch(t)-2 \leq \epoch(\gjattime[val=v',time=t'']) < \epoch(\gjattime[val=v',time=t']) \leq \epoch(t)$.
            By sub cases.
            \begin{description}
