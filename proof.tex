...
\Proc{$\var[val=v]{\getlatestconfirmed}(b_c)$}{
    ...
    \uIf{$\epoch(\bcand) < \epoch(\var[val=v]{t}) - 1$ {\bf or}
        ...
        {\bf and}
        $\neg (\var[val=v]{\mathit{is\_chain\_one\_confirmed}}(b_c, \guattime[val=v,time=\slotstart(\prevfirstslotepoch{t})]))$ {\bf and} $b_c \succeq \guattime[val=v,time=\slotstart(\slot(t)-1)]$\label{ln:if-bcand-npreceq-head}}{
