...
\Fn{highestConfirmedDesc$_v(\bconfirmed, s)$}{
    $b_\mathsf{highest\_lmd\_confirmed} \gets \highestLMDConfirmedDesc_v(\bconfirmed, s)$\;    
    \uIf{$\epoch(b_\mathsf{highest\_lmd\_confirmed}) > \epoch(\bconfirmed)$ \textbf{and}
        $\neg \willChkpBeJustified_v(\chkp(\head), s)$} {
        $\blmdconfirmed \gets \max\left(\left\{b' \in \chain(b_\mathsf{highest\_lmd\_confirmed}) : \epoch(b') = \epoch(s)-1\right\}\right)$
    }
    \uElse {
        $\blmdconfirmed \gets b_\mathsf{highest\_lmd\_confirmed}$
    }

    \uIf{$\epoch(s) = \epoch(\blmdconfirmed)$ \textbf{or}
        $s = \firstslot(epoch(s))$ \textbf{or}
        $\willNoConflictingChkpBeJustified_v(C(\head, \epoch(s), s))$} {

        $canidates \gets \left\{b' \in \chain(\blmdconfirmed) : b' \succ \bconfirmed \wedge
        \willNotBeFilteredOut_v(b', s)\right\}$;\\        
        \uIf{$canidates \ne \emptyset$}{
